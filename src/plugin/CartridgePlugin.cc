...
public:
   ...
   uint8_t OnOpenMsxPeekMem(uint16_t address, uint16_t pc, uint64_t time) const override { return 0; };
	const uint8_t* OnOpenMsxGetReadCacheLine(uint16_t start) const override { return nullptr; };
   uint8_t OnOpenMsxReadMem(uint16_t address, uint16_t pc, uint16_t time) override { return 0; };
   ...
   uint8_t OnOpenMsxReadIO(uint16_t port, uint16_t pc, uint16_t time) override { return 0; };
	uint8_t OnOpenMsxPeekIO(uint16_t port, uint16_t time) const override { return 0; };
...
void CartridgePlugin::RegisterEvents(CartridgePluginEventInterface *evt_)
{
	if (evt_ == nullptr)
	{
...
{
   return evt->OnOpenMsxPeekMem(address, getCPUInterface().GetMarkedPc(), time.time);
}
...
{
	return evt->OnOpenMsxPeekIO(port, time.time);
}
