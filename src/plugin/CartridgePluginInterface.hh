...
typedef OPENMSXEXPORT DWORD (*tOpenMsxPluginEntry) (CartridgePluginInterface *);
...
public:
   ...
   virtual uint8_t  OnOpenMsxReadMem(uint16_t address, uint16_t pc, uint16_t time) { return 0; };
   virtual void     OnOpenMsxWriteMem(uint16_t address, uint8_t value, uint16_t pc, uint64_t time) {};
   virtual uint8_t  OnOpenMsxPeekMem(uint16_t address, uint16_t pc, uint64_t time) const { return 0; };
   ...
   virtual uint8_t  OnOpenMsxPeekIO(uint16_t port, uint16_t time) const { return 0xff; };
